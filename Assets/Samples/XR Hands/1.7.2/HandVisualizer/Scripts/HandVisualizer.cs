...
// Hand rig setups can differ between platforms. In these cases, the
// HandVisualizer supports displaying unique hands on a per-platform basis. If
// you would like to customize the hand meshes that are displayed by the
// HandVisualizer, based on the platform you are using, you will need to replace
// the rigged hand mesh references assigned to the corresponding fields for that
// platform. For Meta Quest devices, assign your rigged hand meshes to the
// "m_MetaQuestLeftHandMesh" & "m_MetaQuestRightHandMesh" fields. For Android XR
// devices, assign your rigged hand meshes to the "m_AndroidXRLeftHandMesh" &
// "m_AndroidXRRightHandMesh" fields. The rigged hand meshes that are assigned
// for a given platform will be displayed when that platform is detected, and
// any other rigged hand meshes assigned for other undetected platforms will not
// be displayed.

/// <summary>
/// This component visualizes the hand joints and mesh for the left and right
/// hands.
/// </summary>
public class HandVisualizer : MonoBehaviour {
  ...
  [SerializeField]
  [Tooltip("If this is enabled, this component will enable the Input System " +
           "internal feature flag 'USE_OPTIMIZED_CONTROLS'. You must have at " +
           "least version 1.5.0 of the Input System and have its backend " +
           "enabled for this to take effect.")]
  bool m_UseOptimizedControls;

  [SerializeField, FormerlySerializedAs("m_LeftHandMesh")]
  [Tooltip("References either a prefab or a GameObject in the scene that " +
           "will be used to visualize the left hand.")]
  GameObject m_MetaQuestLeftHandMesh;

  [SerializeField, FormerlySerializedAs("m_RightHandMesh")]
  [Tooltip("References either a prefab or a GameObject in the scene that " +
           "will be used to visualize the right hand.")]
  GameObject m_MetaQuestRightHandMesh;

  [SerializeField]
  [Tooltip("References either a prefab or a GameObject in the scene that " +
           "will be used to visualize the left hand on Android XR devices." +
           "<br><br><b>Instructions for how to setup and use these meshes " +
           "can be found at the top of the <b>HandVisualizer.cs class</b>")]
  GameObject m_AndroidXRLeftHandMesh;

  [SerializeField]
  [Tooltip("References either a prefab or a GameObject in the scene that " +
           "will be used to visualize the right hand on Android XR devices." +
           "<br><br><b>Instructions for how to setup and use these meshes " +
           "can be found at the top of the <b>HandVisualizer.cs class</b>")]
  GameObject m_AndroidXRRightHandMesh;

  [SerializeField]
  [Tooltip("(Optional) If this is set, the hand meshes will be assigned this " +
           "material.")]
  ...
  [SerializeField][Tooltip("The prefab that will be used to visualize the " +
                           "joints for debugging.")] GameObject
      m_DebugDrawPrefab;
  ...
  [SerializeField][Tooltip(
      "Prefab to use for visualizing the velocity.")] GameObject
      m_VelocityPrefab;
  ...
  protected void OnEnable() {
    ...
    UpdateRenderingVisibility(m_LeftHandGameObjects,
                              m_Subsystem.leftHand.isTracked);
    UpdateRenderingVisibility(m_RightHandGameObjects,
  ...
  protected void Update() {
    ...
    if (m_Subsystem.detectedHandMeshLayout ==
    ...
    if (m_LeftHandGameObjects == null) {
      m_LeftHandGameObjects = new HandGameObjects(
          Handedness.Left, transform, selectedLeftHandMesh, m_HandMeshMaterial,
          m_DebugDrawPrefab, m_VelocityPrefab);
    }

    if (m_RightHandGameObjects == null) {
      m_RightHandGameObjects = new HandGameObjects(
          Handedness.Right, transform, selectedRightHandMesh,
          m_HandMeshMaterial, m_DebugDrawPrefab, m_VelocityPrefab);
    }
  ...
  void UpdateRenderingVisibility(HandGameObjects handGameObjects,
                                 bool isTracked) {
    ...
    handGameObjects.SetVelocityType(isTracked ? m_VelocityType
                                              : VelocityType.None);
  }
  ...
  void OnUpdatedHands(XRHandSubsystem subsystem,
    ...
    m_LeftHandGameObjects.UpdateJoints(
        ...
        m_DebugDrawJoints, m_VelocityType);
  ...
  class HandGameObjects {
    ...
    GameObject[] m_DrawJoints =
        new GameObject[XRHandJointID.EndMarker.ToIndex()];
    GameObject[] m_VelocityParents =
        new GameObject[XRHandJointID.EndMarker.ToIndex()];
    LineRenderer[] m_Lines =
        new LineRenderer[XRHandJointID.EndMarker.ToIndex()];
    JointVisualizer[] m_JointVisualizers =
    ...
    public HandGameObjects(Handedness handedness, Transform parent,
                           GameObject meshPrefab, Material meshMaterial,
                           GameObject debugDrawPrefab,
                           GameObject velocityPrefab) {
      void AssignJoint(XRHandJointID jointId, Transform jointDrivenTransform,
        ...
        m_Lines[jointIndex] =
            m_DrawJoints[jointIndex].GetComponent<LineRenderer>();
        m_Lines[jointIndex].startWidth = m_Lines[jointIndex].endWidth =
            k_LineWidth;
        s_LinePointsReuse[0] = s_LinePointsReuse[1] =
            jointDrivenTransform.position;
        m_Lines[jointIndex].SetPositions(s_LinePointsReuse);

        if (m_DrawJoints[jointIndex].TryGetComponent<JointVisualizer>(
                out var jointVisualizer))
      ...
      m_HandRoot.SetActive(
          false); // Deactivate so that added components do not run OnEnable
                  // before they are finished being set up
      ...
      if (m_MeshController == null) {
        ...
        for (var childIndex = 0; childIndex < m_HandRoot.transform.childCount;
             ++childIndex) {
          var childTransform = m_HandRoot.transform.GetChild(childIndex);
          if (childTransform.TryGetComponent<SkinnedMeshRenderer>(
                  out var renderer))
      ...
      for (var i = 0; i < skeletonDriver.jointTransformReferences.Count; i++) {
        var jointTransformReference =
            skeletonDriver.jointTransformReferences[i];
    ...
    public void OnDestroy() {
      ...
      for (var jointIndex = 0; jointIndex < m_VelocityParents.Length;
           ++jointIndex) {
    ...
    public void SetVelocityType(VelocityType velocityType) {
      for (int jointIndex = 0; jointIndex < m_VelocityParents.Length;
           ++jointIndex)
        ToggleRenderers<LineRenderer>(velocityType != VelocityType.None,
                                      m_VelocityParents[jointIndex].transform);
    }

    public void UpdateJoints(XRHand hand, bool areJointsTracked,
                             bool debugDrawJoints, VelocityType velocityType) {
      if (!areJointsTracked)
        return;
      ...
      UpdateJoint(debugDrawJoints, velocityType,
                  hand.GetJoint(XRHandJointID.Wrist), ref wristPose,
                  ref parentIndex);
      UpdateJoint(debugDrawJoints, velocityType,
                  hand.GetJoint(XRHandJointID.Palm), ref wristPose,
                  ref parentIndex, false);

      for (var fingerIndex = (int)XRHandFingerID.Thumb;
           fingerIndex <= (int)XRHandFingerID.Little; ++fingerIndex) {
        ...
        for (var jointIndex = fingerId.GetFrontJointID().ToIndex();
             jointIndex <= jointIndexBack; ++jointIndex) {
          UpdateJoint(debugDrawJoints, velocityType,
                      hand.GetJoint(XRHandJointIDUtility.FromIndex(jointIndex)),
                      ref parentPose, ref parentIndex);
        }
      }
    }

    void UpdateJoint(bool debugDrawJoints, VelocityType velocityType,
                     XRHandJoint joint, ref Pose parentPose,
                     ref int parentIndex, bool cacheParentPose = true) {
      ...
      if (velocityType != VelocityType.None &&
          m_VelocityParents[jointIndex].TryGetComponent<LineRenderer>(
              out var renderer)) {
        m_VelocityParents[jointIndex].transform.localPosition = Vector3.zero;
        m_VelocityParents[jointIndex].transform.localRotation =
            Quaternion.identity;

        s_LinePointsReuse[0] = s_LinePointsReuse[1] =
            m_VelocityParents[jointIndex].transform.position;
    ...
    static void ToggleRenderers<TRenderer>(bool toggle,
                                           Transform rendererTransform)
        where TRenderer : Renderer {
      if (rendererTransform.TryGetComponent<TRenderer>(out var renderer))
        renderer.enabled = toggle;

      for (var childIndex = 0; childIndex < rendererTransform.childCount;
           ++childIndex)
        ToggleRenderers<TRenderer>(toggle,
                                   rendererTransform.GetChild(childIndex));
    }
  }
}
}
